<div id="content_for_layout">

	<div class="floating" id="accountactive">
		<h3>Account Activated</h3>
		<p>You PheedBakk account has been successfully activated</p>
		<p>You can now login and share today</p>
		<p>Please visit the help section, to help you get aquitainted with PheedBakk</p>
		<p>You can also send us feedback, to helps us make PheedBakk better for you</p>
		<p>Thank you for using PheedBakk</p>
        <p><?php echo anchor("","Login",array("class"=>"submit_btn_blue")); ?></p>
	</div>
	
</div>